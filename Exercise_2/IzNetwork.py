    ...
    def NeuronUpdate(self, i, t):
        ...
        for j in xrange(self.Nlayers):
            ...
            if j in self.layer[i].S:
                # target neuron->rows, source neuron->columns
                S = self.layer[i].S[j]
        ...
        for k in xrange(int(1 / dt)):
            ...
            self.layer[i].v += dt * \
                (0.04 * v * v + 5 * v + 140 - u + self.layer[i].I)
            self.layer[i].u += dt * \
                (self.layer[i].a * (self.layer[i].b * v - u))
            ...
            if len(fired) > 0:
                for f in fired:
                    ...
                    if len(self.layer[i].firings) != 0:
                        self.layer[i].firings = np.vstack(
